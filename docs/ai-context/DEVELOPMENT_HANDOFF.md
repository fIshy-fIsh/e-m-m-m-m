...
- **Valuation Budget Calibration Phase 15B:** **POLICY FREEZE COMPLETE / MERGED** via PR #6. Decision artifact: `research/valuation_budget_calibration/POLICY_DECISION.md`; default remains `5`; hard max remains `60`; designed shares are not production probabilities. Canonical main after Phase 15A/15B = `7a73cc026f93bbed9d9c089c96e6565a6c43c68d`; main CI run `33350081125` SUCCESS.
- **Phase 15C-1 — Representative snapshot calibration design freeze:** **COMPLETE at `2af6d4c` / CI run `33352295570` SUCCESS**. Balanced eight-stratum frame; exact current ten-goods cohort-depth rebuild per observation; proposed 14 UTC days / 112 attempts / minimum 96 COMPLETE / 10 per stratum / 12 dates; immutable normalized JSON + append-only hashed manifest; live datasets outside Git by default. Narrow current page-1/default-sort interface is sufficient; broader scope remains `PHASE15C1_LIVE_COLLECTION_INTERFACE_GAP`.
- **Phase 15C-2 — Research collector and offline replay:** **IMPLEMENTATION COMPLETE at `a869cd09b476d3ecc01541d7bacef2d8c695e57c`**. Deterministic planner, strict schema/redaction, immutable storage/SHA, append-only manifest, exact ten-page collector, COMPLETE-only offline replay, and one-shot CLI; no production/valuation/cache/risk/opportunity/scheduler/budget path.
- **Phase 15C-2B — Bounded live snapshot interface smoke:** **COMPLETE**. Fixed campaign `phase15c2b-non-policy-smoke-a869cd09`, Industrial Grade / normal, nominal `2026-08-31T15:17Z`, scheduled `15:25Z`, valid until `15:55Z`; exactly 10 BUFF page-1/default-sort requests; COMPLETE. Manifest/schema/canonical bytes/SHA PASS; socket-disabled offline replay PASS: recipes `2`, run-unique `45`, per-recipe `[45,35]`, recipe-2 incremental `0`, states `2`. Artifact outside Git; no listing-level content recorded; SteamDT/Redis/cache/valuation/EV/risk `0`. Interface compatibility only; representative campaign remains NOT STARTED / NOT AUTHORIZED.
...
- Valuation Budget Calibration Phase 15B is **POLICY FREEZE COMPLETE / MERGED** via PR #6: production default remains `5`; hard max remains `60`.
- Phase 15C-1 design, Phase 15C-2 research implementation, and the single Phase 15C-2B non-policy interface smoke are **COMPLETE** on `feature/representative-snapshot-calibration`.
- Representative 14-day / 112-attempt campaign, scheduler/background collection, and numeric policy change remain **NOT STARTED / NOT AUTHORIZED**.
...
- Representative campaign execution remains blocked on separate explicit authorization; Phase 15C-2 implements one-shot tooling only and adds no scheduler/background collector.
- The frozen narrow sampling frame can use the current page-1/default-sort anonymous compatibility path. Any broader full-market/order-book/pagination-complete claim remains blocked by `PHASE15C1_LIVE_COLLECTION_INTERFACE_GAP`.
- Any numeric policy recommendation requires the frozen valid-count/time/stratum/reproducibility gates; any hard-max change additionally requires separate external-call safety review.
