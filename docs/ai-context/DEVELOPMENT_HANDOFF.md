...
- **Phase 15C-1 — Representative snapshot calibration design freeze:** **COMPLETE on `feature/representative-snapshot-calibration`**. Balanced eight-stratum frame; exact current ten-goods cohort-depth rebuild per observation; proposed 14 UTC days / 112 attempts / minimum 96 COMPLETE / 10 per stratum / 12 dates; immutable normalized JSON + append-only hashed manifest; live datasets outside Git by default. Narrow current page-1/default-sort interface is sufficient; full-market/order-book/pagination-complete scope remains `PHASE15C1_LIVE_COLLECTION_INTERFACE_GAP`. No live collection, production/test/script/workflow change, scheduler, or numeric policy change.
...
- Phase 15C-1 is **DESIGN FREEZE COMPLETE** on `feature/representative-snapshot-calibration`; Phase 15C-2 collector/replay implementation and live collection remain **NOT STARTED / NOT AUTHORIZED**.
...
- Phase 15C-2 remains blocked on explicit post-design review and authorization; no collector, replay implementation, scheduler, or live request is authorized by Phase 15C-1.
- The frozen narrow sampling frame can use the current page-1/default-sort anonymous compatibility path. Any broader full-market/order-book/pagination-complete claim is blocked by `PHASE15C1_LIVE_COLLECTION_INTERFACE_GAP` until separately confirmed.
