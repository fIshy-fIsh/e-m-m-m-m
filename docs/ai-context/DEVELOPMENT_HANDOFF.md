...
- **Phase 15C-2 — Research collector and offline replay:** **IMPLEMENTATION COMPLETE on the same branch**. Adds deterministic planner, strict schema/redaction guard, atomic immutable storage/SHA, append-only manifest, exact ten-page sequential collector with minimum two-second starts/no retry, COMPLETE-only pinned-provenance replay at fixed `2 / 256`, plan-only and explicit one-shot CLI, and focused tests. No production/valuation/cache/risk/opportunity/scheduler/budget path. Smoke NOT_EXECUTED because commit-derived Industrial Grade/normal had no eligible frozen 30-minute slot; zero requests/artifacts. Representative campaign remains NOT STARTED / NOT AUTHORIZED.
...
- Phase 15C-1 is **DESIGN FREEZE COMPLETE** at `2af6d4c`; Phase 15C-2 research collector/replay implementation is **COMPLETE** on `feature/representative-snapshot-calibration`.
