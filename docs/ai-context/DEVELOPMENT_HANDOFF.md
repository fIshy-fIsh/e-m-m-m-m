...
- **Branch:** `feature/scanner-valuation-integration` (canonical development line; Phase 14A design freeze). Tracked main is `main` at P3 (`24c95c029f583d5cc0b0a67986e48c06d0ef7957`); `main` is unchanged by Phase 14A.
- **HEAD:** the Phase 14A doc-only commit `freeze scanner valuation integration design` (current `feature/scanner-valuation-integration` HEAD). Upstream HEAD = local HEAD after push; `ahead/behind = 0 0`.
- **HEAD message:** `freeze scanner valuation integration design` (design-only).
- **Phase:** `PHASE_14A_COMPLETE` (design freeze; no production code change).
...
- Phase 14A — Scanner Valuation Integration Design Freeze: **IN PROGRESS — DESIGN FREEZE (no code, no commit to `main`)**. `specs/2026-08-29-scanner-valuation-integration-design-freeze/{requirements,plan,validation}.md` finalized the scanner-owned `RunScopedValuationSession` boundary, `PriceCacheReadPolicy.FRESH_ONLY` initial cache policy, run-scoped exact-name memo, additive counter migration (Option A preferred), atomic live-demand preflight (`max_valuation_requests_per_run` now means NEW LIVE SteamDT provider demand, not structural recipe demand), failure reuse within a run, no partial valuation, no scheduler, no Redis requirement for the default one-shot CLI. Six new decision IDs appended: `D-CACHE-002..004`, `D-BUDGET-001`, `D-ACCOUNTING-001`, `D-PHASE14A-COMPLETE`. `D-CACHE-001` remains `Active` (the runtime cache is still not implemented); Phase 14A only freezes the design. Branch: `feature/scanner-valuation-integration`. The future 14B / 14C / 14D implementation phases are PROPOSED / NOT AUTHORIZED.
- Phase 14A-R1 — Design Coherence Correction: **IN PROGRESS — DOCS ONLY (no code, no commit to `main`)**. `D-PHASE14A-R1-COHERENCE` records nine sub-corrections to the Phase 14A prose found during post-review: (1) `select_steamdt_price_quote` is generic cross-platform — strict BUFF adapter must be composed at the session level via reuse/adaption of `select_buff_output_price`; (2) explicit two-stage prepare/execute session contract (Stage A `prepare_output_prices` issues zero live calls; Stage B `resolve_prepared` only after atomic-cap admission); (3) cache backend/codec/adapter errors propagate by identity and are NOT live candidates; (4) `SteamDTBuffPriceProvider.get_prices` converts ordinary failures to `PriceLookupResult.missing/errors` (only `MemoryError` and uncatchable `BaseException` propagate by identity); (5) counter contract finalized under Option A — legacy semantics preserved exactly; (6) 14B reuse test corrected (Recipe2 memo hits = 9, not 0); (7) NO scanner fresh_ttl numeric default frozen — 5-minute value is historical precedent only; (8) initial Phase 14C is READ-only (no scanner writeback); (9) `D-CACHE-001` remains Active until Phase 14B / 14C land and are verified. Phase 14B / 14C / 14D remain PROPOSED / NOT AUTHORIZED UNTIL R1 REVIEW. Branch: `feature/scanner-valuation-integration`.

## Next Action (ordered)

- **No new product-development phase is currently authorized.** Phase 14A and Phase 14A-R1 are design freezes; 14B / 14C / 14D remain PROPOSED / NOT AUTHORIZED UNTIL R1 REVIEW.
...
- Scanner Valuation Integration: Phase 14A (Scanner Valuation Integration Design Freeze) is COMPLETE (docs only). The frozen design lives at `specs/2026-08-29-scanner-valuation-integration-design-freeze/{requirements,plan,validation}.md` and the six new decision IDs `D-CACHE-002..004`, `D-BUDGET-001`, `D-ACCOUNTING-001`, `D-PHASE14A-COMPLETE`. Phase 14B / 14C / 14D (run-scoped exact-name reuse; Phase 12D cache integration with `FRESH_ONLY`; CLI + scale/live validation) are PROPOSED / NOT AUTHORIZED.
...
- None for the completed Phase 14A design freeze.
- Phase 14B / 14C / 14D are PROPOSED and require explicit authorization before any code, test, or CLI change.
