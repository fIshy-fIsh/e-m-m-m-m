...
Post-Phase-14A design-freeze baseline:    (the Phase 14A doc-only commit
                                            `freeze scanner valuation integration design`,
                                            current HEAD of `feature/scanner-valuation-integration`;
                                            same commit records D-PHASE14A-COMPLETE and the
                                            six new decision IDs `D-CACHE-002..004`,
                                            `D-BUDGET-001`, `D-ACCOUNTING-001`,
                                            `D-PHASE14A-COMPLETE`; verify SHA via
                                            `git rev-parse HEAD` at task entry)
...
- **Phase:** `PHASE_14A_COMPLETE`. Phase 13T-1 through 13T-4A remain committed and pushed (latest production commit `9288794`); Phase 13T-4B is a live-only validation performed against that commit and intentionally produced no commit and no repository artifact; Phase 14A is a design freeze (no production code, no Protected Core edit, no live request).
...
- **Run-level SteamDT output-price cache status:** **NOT IMPLEMENTED** (`D-CACHE-001` Active). Same exact output name in separate recipe valuation calls is a separate logical request today. Phase 14A (design freeze at `specs/2026-08-29-scanner-valuation-integration-design-freeze/`) has frozen the design for any future Phase 14 implementation: scanner-owned `RunScopedValuationSession` boundary, run-scoped exact-name memo, `PriceCacheReadPolicy.FRESH_ONLY` initial cache policy, additive counter migration (Option A), atomic live-demand preflight, failure reuse within run, no automatic same-name retry, no partial valuation, no fallback, no probability renormalization, no scheduler. Phase 14A does NOT promote the feature; `D-CACHE-001` remains `Active` until Phase 14B lands and is verified.
...
- **Working tree:** contains only the two pre-existing excluded research JSON files (`research/identity_revalidation/data/modest_serhat.json`, `research/identity_revalidation/data/timofey_ivanenko.json`). All Phase 13T-1 through 13T-4A work is committed and pushed at `9288794`. Phase 14A design-freeze commit (`freeze scanner valuation integration design`) is the local HEAD of `feature/scanner-valuation-integration`; `main` is unchanged.
