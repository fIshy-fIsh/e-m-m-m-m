...
- **Phase:** `PHASE_14D_COMPLETE`. Phase 14B implemented run-scoped exact-name reuse and NEW-LIVE accounting. Phase 14C integrated optional Phase12D FRESH_ONLY cache READ at the scanner service/session boundary. Phase 14D wires the default one-shot CLI cache composition with in-memory default and optional Redis through the existing factory.
