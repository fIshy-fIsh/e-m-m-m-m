...
Phase 14B checkpoint:                    current feature-branch HEAD
                                            add run-scoped scanner valuation reuse
...
- **Phase:** `PHASE_14B_COMPLETE`. Phase 14A (`e98cd97`) and Phase 14A-R1 (`bb056e5`) froze/corrected the design. Phase 14B is the first production migration: run-scoped exact-name valuation reuse and NEW-LIVE budget accounting, with no persistent Phase 12D cache integration.
- **Latest completed phases:**
  - **Phase 14B — Run-scoped exact-name valuation reuse** — scanner-owned `app/services/scanner_valuation_session.py`; fresh session inside every `LiveScannerOrchestrator.run_once()`; immutable session-bound two-stage plans; async prepare (memo only, zero provider calls); atomic NEW-LIVE exact-name cap admission; execute only NEW exact names; memo success + terminal failure reuse; no same-name retry; no cross-run reuse; existing `ValuationService` formula application reused through a session-local fixed provider; ordinary generic-provider errors bounded/fail-closed; `MemoryError` and uncaught `BaseException` identity preserved. Legacy logical valuation counters unchanged; additive `run_reuse_*`, `live_*`, and zero-valued 14C `cache_*` counters added. Deep-pool same-output fixture: 20 logical requests, 10 provider exact-name demand, 10 run-reuse hits; cap 10 values both recipes; cap 9 blocks both with zero provider calls. Full offline validation: 3382 passed, 23 skipped, 1 warning. No Phase 12D cache module imported or modified. `D-CACHE-001` remains Active as the broader persistent-cache migration record; Phase 14C remains open.
...
- **Run-level exact-name valuation reuse status:** **IMPLEMENTED** (Phase 14B). A fresh scanner-owned session is created inside each `run_once()`; exact-name successes and terminal failures are reused across recipes in that run only. Blocked names are not memoized. `max_valuation_requests_per_run` counts NEW LIVE exact-name demand. The broader `D-CACHE-001` record remains Active because persistent Phase 12D scanner-cache integration is still **NOT IMPLEMENTED**. FRESH_ONLY scanner cache reads and scanner write-after-live remain **NOT IMPLEMENTED**; initial Phase 14C is READ-only.
...
- **Working tree:** contains only the two pre-existing excluded research JSON files after the Phase 14B commit is created/pushed. Phase 14B implementation lives on `feature/scanner-valuation-integration`; canonical `main` remains P3 (`24c95c0...`). Verify live via Git.
