...
- **Status:** Active protocol. Phase 15C-2 research-only collector/replay implementation completed under `D-VALUATION-BUDGET-SNAPSHOT-IMPLEMENTATION-001`; representative campaign remains NOT STARTED / NOT AUTHORIZED.
...
- **Scope:** design/spec/docs/research protocol/example only in Phase 15C-1; no production/test/script/workflow/config/dependency edit, live request/data, scheduler, valuation, Redis, cache writeback, or budget change.
- **Next:** Phase 15C-2 implemented the frozen research tooling under `D-VALUATION-BUDGET-SNAPSHOT-IMPLEMENTATION-001`; campaign execution remains separately gated.

## D-VALUATION-BUDGET-SNAPSHOT-IMPLEMENTATION-001 — Frozen snapshot collector and offline replay implemented

- **Date:** 2026-08-31 (Phase 15C-2)
- **Status:** Active implementation checkpoint on `feature/representative-snapshot-calibration`; representative 14-day campaign NOT STARTED / NOT AUTHORIZED.
- **Authority:** Phase 15C-1 checkpoint `2af6d4c562eaa1dbfeee2e2b7f73e2959c9d647b`; branch CI run `33352295570` / job `quality` SUCCESS; protocol `representative-listing-snapshot-v1` unchanged.
- **Implementation:** research-only `snapshot_protocol.py`, `snapshot_schema.py`, `snapshot_storage.py`, `snapshot_collector.py`, `snapshot_replay.py`, and `capture_snapshot_once.py`. Production `app/**` remains unchanged.
- **Planner/schema:** exact frozen SHA-256 snapshot ID and jitter; productive-stratum validation; current exact 10-goods cohort-depth plan; strict required/unknown/duplicate-key/exact-type/Decimal/UTC/order/count/status/provenance invariants; persisted key/value secret/header/query/raw-payload/seller/account material fails closed without value disclosure.
- **Storage:** canonical sorted UTF-8 JSON with one trailing newline; atomic no-overwrite snapshot creation; SHA-256 final bytes; append-only canonical JSONL manifest; manifest-only missed and universe-planning failures; corrections append superseding records.
- **Collector:** exactly one injected provider call per planned goods ID, ten total, sequential; public minimum two-second request-start interval; no retry/poll/loop; existing exact identity, canonical intrinsic, metadata, candidate boundaries applied after each fetched page. Fetch/parse failures yield PARTIAL; binding/catalog/provenance contradiction yields INVALID; all-success/empty exact pages yield COMPLETE. Collection stops before valuation, SteamDT, cache/Redis, EV/risk, opportunity, or purchase paths.
- **Replay:** strict duplicate-key schema load; optional snapshot hash verification; pinned identity/metadata path+SHA and snapshot validation; COMPLETE-only policy-facing replay; accepted input reconstruction; unchanged scanner composition and solver at fixed `2 / 256`; Phase 15A exact-name helper reused. COMPLETE zero-recipe observations remain in the denominator with `recipe_count=0` and `run_unique_output_names=0`.
- **CLI:** module entrypoint supports one plan-only or explicit one-shot non-policy smoke execution; plan-only constructs no HTTP client and reports zero requests; live mode requires campaign ID containing `non-policy-smoke`, artifact root outside Git, and frozen scheduled+30-minute execution window. It never schedules, retries, loops, polls, or reads `.env`.
- **Validation:** focused schema/determinism/storage/redaction/collector/pacing/replay/provenance/CLI/AST suite passed (`29` tests); Phase 15A calibration regression passed (`9`); protected scale suites passed (`4 + 20`); Ruff and mypy-app passed; diff check clean. No full pytest was required because production code was unchanged.
- **Bounded smoke at implementation checkpoint:** `NOT_EXECUTED`. Deterministic stratum selection from the Phase 15C-1 commit chose Industrial Grade / normal, but that initial attempt had no eligible frozen window; zero requests. Phase 15C-2B later executed one separately fixed-window smoke under `D-VALUATION-BUDGET-SNAPSHOT-SMOKE-001`.
- **Frozen scope:** no production/app, workflow, dependency, `.env.example`, default/hard-max/CLI-production, atomic NEW-LIVE, valuation/cache/risk/opportunity, scheduler/background, purchase/trade, login/cookie, or bypass change. Default remains `5`; hard max remains `60`; `PHASE15C1_LIVE_COLLECTION_INTERFACE_GAP` remains controlling outside the narrow page-1/default-sort frame.
- **Next gate:** representative campaign execution remains NOT STARTED / NOT AUTHORIZED and requires separate explicit authorization. This checkpoint alone provides no production-frequency evidence and authorizes no numeric policy change.

## D-VALUATION-BUDGET-SNAPSHOT-SMOKE-001 — One fixed-window live snapshot interface smoke verified

- **Date:** 2026-08-31 (Phase 15C-2B)
- **Status:** Active interface-compatibility evidence only; not representative or policy evidence.
- **Baseline:** branch `feature/representative-snapshot-calibration` at `a869cd09b476d3ecc01541d7bacef2d8c695e57c`; upstream `0/0`; canonical main `7a73cc026f93bbed9d9c089c96e6565a6c43c68d`; Phase 15C-2 CI run `33355386396` / `quality` SUCCESS.
- **Fixed window:** campaign `phase15c2b-non-policy-smoke-a869cd09`; Industrial Grade / normal; nominal `2026-08-31T15:17:00Z`; frozen jitter scheduled `15:25:00Z`; valid through `15:55:00Z`. Stratum/campaign/window were fixed before market results and were not changed.
- **Pre-live gates:** focused Phase 15C-2 `29`, Phase 15A `9`, protected scale `4 + 20`, Ruff, mypy-app, and diff check all passed immediately before execution.
- **Network boundary:** exactly one observation attempt; exactly 10 sequential anonymous BUFF page-1/default-sort requests; collector-enforced minimum two seconds between request starts; no retry, polling, pagination, second observation, second stratum, login, cookie, secret inspection, or `.env` inspection.
- **Outcome:** `COMPLETE`. Artifact/manifest stored outside Git in OS temporary artifact storage. No raw payload or listing-level facts were committed or recorded in project docs.
- **Validation:** append-only manifest PASS; strict duplicate-key/schema/status/count validation PASS; canonical sorted UTF-8 bytes PASS; snapshot SHA-256 PASS; offline replay under disabled socket constructors PASS with zero network calls.
- **Safe aggregate replay:** `recipe_count=2`; `run_unique_output_names=45`; per-recipe unique counts `[45,35]`; recipe-2 incremental NEW `0`; composition states explored `2`.
- **Zero-use boundaries:** SteamDT `0`; Redis/cache `0`; valuation/EV/risk/opportunity `0`; scheduler/background campaign `0`; purchase/trade/login/cookie `0`.
- **Interpretation:** proves only compatibility of the frozen narrow anonymous first-page/default-sort interface with normalized immutable storage and offline replay. It does not establish market frequency, production weighting, cap coverage, expected workload, profitability, representative evidence, or a budget-policy basis.
- **Policy/campaign:** representative 14-day / 112-attempt campaign remains NOT STARTED / NOT AUTHORIZED. Production default remains `5`; hard max remains `60`; `NO_PRODUCTION_DEFAULT_CHANGE_PENDING_REPRESENTATIVE_SNAPSHOT`, `HARD_MAX_60_REVIEW_DEFERRED`, and the broader `PHASE15C1_LIVE_COLLECTION_INTERFACE_GAP` remain controlling.
