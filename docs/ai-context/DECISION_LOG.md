...
- **Status:** Active as the broader scanner-cache migration record. Historical Phase 13T state below remains authoritative for Phase 13T. The run-scoped cross-recipe exact-name reuse portion migrated in Phase 14B; persistent Phase 12D scanner-cache integration remains **not implemented**.
...
- **Future revisit:** Phase 14B has now implemented and verified the run-scoped exact-name success/failure memo and NEW-LIVE budget migration. `D-CACHE-001` remains Active until Phase 14C integrates persistent Phase 12D FRESH_ONLY scanner reads and is verified. Initial Phase 14C excludes scanner write-after-live.
...
- **Phase 14B implementation note:** the run-scoped exact-name reuse portion is implemented and verified by `D-PHASE14B-COMPLETE`. The broader `D-CACHE-001` record remains Active only for persistent Phase 12D scanner-cache integration (Phase 14C).
...
- **Decision:** Phase 14A-R1 corrects nine internal design contradictions found during post-Phase-14A review. R1 is docs/spec/decision-log only; it does not promote Phase 14A to "implemented" and does not start Phase 14B implementation. The corrections are:
  ...
  7. **TTL numeric default not frozen.** NO scanner `fresh_ttl` numeric default is frozen in Phase 14A-R1. The 5-minute value at `scripts/steamdt_refresh_integration.py:59` (`INTEGRATION_POLICY = PriceCachePolicy(fresh_ttl=timedelta(minutes=5))`) is **historical manual-script precedent only**, not a scanner default. Phase 14C will explicitly choose and document the scanner TTL under implementation authorization. InMemory and Redis use identical policy semantics once chosen.
  ...
  9. **`D-CACHE-001` remains Active.** R1 does not reclassify `D-CACHE-001`. The runtime cache is still not implemented. Phase 14B (run-scope reuse) and Phase 14C (Phase 12D scanner cache integration) are the phases that, when they land AND are verified, reclassify `D-CACHE-001` from `Active` to `Implemented`. Until those phases land and are verified, the six public surfaces that say "run-level cross-recipe exact-price reuse = NOT IMPLEMENTED" continue to say so.
- **What is unchanged:** the Phase 14A spec trilogy location (`specs/2026-08-29-scanner-valuation-integration-design-freeze/`); the pointer surfaces (`PHASE_14A_COMPLETE`); the canonical main pointer (`P3 = 24c95c029f583d5cc0b0a67986e48c06d0ef7957`); the local-only tag `v1-dry-run-baseline`; the two protected research JSONs.
...
- **Future revisit:** Phase 14B was subsequently explicitly authorized, implemented, tested, and closed by `D-PHASE14B-COMPLETE`. Phase 14C / 14D remain separately authorized future work; the corrected R1 contracts remain binding.
...
- **D-CACHE-001:** remains Active as the broader scanner-cache migration record. Its run-scoped reuse portion is complete in Phase 14B. Persistent Phase 12D scanner cache / FRESH_ONLY reads remain not implemented and belong to Phase 14C. Scanner write-after-live is not implemented and remains out of scope for initial 14C.
- **Next:** Phase 14C — Phase12D FRESH_ONLY cache READ integration — NOT STARTED / NOT AUTHORIZED. Phase 14D follows separately.
