...
- **Bounded smoke:** `NOT_EXECUTED`. Deterministic stratum selection from the Phase 15C-1 commit chose Industrial Grade / normal. The commit-derived campaign had no nominal slot whose frozen jittered schedule was within the required 30-minute execution window at smoke time. Protocol miss semantics were honored: zero BUFF requests, no snapshot/manifest artifact, no representative or policy claim.
