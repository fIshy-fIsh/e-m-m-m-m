...
## Phase 12E4B — Qualified Listing to Solver Candidate Adapter Core

`adapt_qualified_buff_listing()` is a pure single-record boundary from the exact existing `BuffListingQualificationResult` to the existing `CandidateListing`. It first rebuilds the supplied result through the public qualification-result constructor, producing an independent validated snapshot without importing qualification private helpers or invoking `BuffListingQualificationService`. It then requires derived `QUALIFIED` status, `FOUND` lookup status with facts, a present eligible decision, equal result/decision candidates and policies, equal lookup/decision facts, quantity at least the result policy threshold, a non-null float, and a non-null authoritative goods ID. Valid `REJECTED` and `MISSING_FACTS` results, legacy-v1 null goods IDs, missing floats, wrong exact types, subclasses, and tampered or inconsistent snapshots all fail closed.

The adapter maps goods ID, listing ID, market name, buy price, paint seed, and observation time directly; `buy_price_cny` remains the exact `Decimal` `price_cny`, and `observed_at` becomes explicit `scanned_at`. The candidate Decimal float is converted exactly once to the legacy `float` representation and then checked for finiteness and inclusive `[0, 1]` range. This lossy type boundary exists only for compatibility with the current `CandidateListing` and recipe-solver contract; the adapter does not round, clamp, stringify, or convert price. It supplies explicit `source="buff"`, `inspect_link=None`, and `raw=None` and never fabricates transport data.

Quantity is validated but one qualification result produces exactly one candidate; available quantity is not expanded. Wear, stickers, facts, policy, eligibility reasons, seller/account data, credentials, URLs, and raw payloads do not enter `CandidateListing`. Goods ID is used only as explicitly supplied provenance and is never derived from listing ID, market name, paint seed, source, a placeholder, a hash, metadata, or any lookup. Metadata remains responsible for StatTrak, Souvenir, collection, rarity, and item float-range information.

Successful repeated calls return equal independently constructed existing candidates and do not mutate qualification state. Ordinary validation, conversion, and destination-construction failures become the fixed `invalid BUFF listing solver adapter contract` error without nested text; `MemoryError`, cancellation, and other `BaseException` control flow propagate unchanged. Public error text and repr expose no listing data, reasons, raw objects, URLs, or secret-shaped values.

This phase does not call a facts provider, evaluator, qualification service, market scanner, metadata provider, or recipe solver. Importing the existing `CandidateListing` necessarily loads its legacy market-scanner module and that module's `BuffClient` type dependency, but the adapter constructs no client and performs no network or authentication work. It does not execute recipes, deduplicate or batch candidates, wire pipeline/scheduler/FastAPI, read config/environment, create tasks or threads, or connect BUFF, SteamDT, or Redis. It adds no live payload mapping, retry/fallback, Discord integration, market write, or automatic purchase and is not production-ready.

## Phase 12E4C — Offline Qualified Listing Adapter Integration

`scripts/buff_listing_solver_adapter_integration.py` is a sibling manual command that composes the existing offline qualification run with the existing strict single-record adapter. It calls `run_qualification_integration()` exactly once, validates its exact immutable run result, traverses ordered qualification results once, and calls `adapt_qualified_buff_listing()` exactly once for each `QUALIFIED` occurrence. `REJECTED` and `MISSING_FACTS` remain normal skipped business outcomes and never reach the adapter.

The frozen, keyword-only, repr-suppressed integration result stores only the complete qualification run result and an ordered tuple of the existing `CandidateListing`; total, qualified, adapted, skipped-rejected, and skipped-missing counts are derived. Qualified order and duplicate occurrences are preserved, available quantity is not expanded, and no listing is deduplicated. The runner does not reconstruct fixture observations, query facts, repeat eligibility, copy adapter field mapping, retry, fall back, run concurrently, continue after failure, or publish partial success. Any ordinary adapter failure becomes the fixed safe `adaptation` stage. Legacy v1 qualified candidates have no authoritative goods ID, so the existing adapter fails closed and the whole command exits 1 rather than silently skipping them.

Both direct and module entrypoints accept the same listing/facts fixture overrides and repository-anchored v2 defaults as the qualification command. Default data produces 4 qualification results, 2 qualified and adapted candidates, 1 skipped rejection, and 1 skipped missing-facts result. Success output reuses the qualification command's public safe candidate-market-name renderer and prints only adapted index, redacted/escaped name, canonical source, float-presence flag, derived counts, and fixed no-solver/no-external-use attestations. It never prints goods or listing IDs, price, numeric float, seed, raw or inspect data, fixture path, object repr, nested exception text, traceback, credentials, or URL-shaped content. Handled failures are atomic and print no partial success summary.

The integration command does not execute the recipe solver, look up metadata, call the market scanner, or wire pipeline, scheduler, FastAPI, or another runtime. Importing the required existing `CandidateListing` still transitively loads the legacy scanner's `BuffClient` type dependency, but no client is constructed and no authentication or network operation occurs. The command sends no BUFF or SteamDT requests, uses no Redis, adds no batch worker, background work, login, Cookie handling, crawler, market write, or automatic purchase, and is not production-ready.
...
Phase 12E2A/E2B/E3A/E3B/E4A/E4B0/E4B/E4C add no live BUFF mapping, HTTP client behavior, endpoint, authentication, signature, login, Cookie handling, crawler, captcha handling, risk-control bypass, browser automation, or automatic purchase. E4A adds only a manual synthetic offline composition of the existing contracts; E4B0 adds only explicit project-owned goods-ID provenance and listing fixture v2 compatibility; E4B adds only the isolated qualified-result-to-existing-candidate adapter; E4C adds only an ordered manual offline composition that skips non-qualified outcomes and still does not execute or wire the recipe solver. These phases do not perform metadata lookup, connect SteamDT or Redis, or wire provider, valuation, pipeline, scheduler, FastAPI, Discord, config, or deployment.
