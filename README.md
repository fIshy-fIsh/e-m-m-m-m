...
- This pure standard-library boundary creates no WebSocket client, dependency, pool, metadata lookup, solver, SteamDT/Redis use, runtime wiring, browser action, or purchase behavior. The supplied contract was re-verified from the three official documentation pages by read-only HTTP retrieval and is recorded with remaining unknowns in `docs/STEAMAPIS_MARKET_DATA_NOTES.md`. It is not production-ready.

### Phase 13A Step 2B SteamApis Candidate Adapter
- `adapt_steamapis_listing_to_candidate()` revalidates one immutable observation and projects it into the existing source-agnostic solver input. It uses the project-owned `steamapis:buff163:<source_offer_id>` compatibility value for both required candidate identities and `steamapis:buff163` as source; these are not authoritative BUFF goods/listing IDs or a SteamApis-documented marketplace ID, and no purchase URL component is parsed.
...
### Phase 13A Step 2E Offline Live Recipe Construction
- `construct_recipe_selections()` adds a source-agnostic trace alongside each existing `ConstructedRecipe`: the ten ordered `CandidateListing.listing_id` values come directly from the same selected internal candidate pairs before identity is discarded into `InputItem`. Existing `ConstructedRecipe`, `construct_recipes()`, and `solve_recipes()` contracts remain compatible, and construction/trade-up calculation still has one authoritative path.
- `construct_live_recipes()` classifies one immutable pool snapshot once, uses only eligible candidates, and processes matching exact rarity/StatTrak/Souvenir buckets independently. A config target of `None` may match both modes but never merges them; collection remains outside the bucket identity, so one constructed recipe may still combine multiple collections.
...
### Phase 13A Step 2K Explicit Opt-In SteamApis Live Offer Smoke
- `scripts/run_live_steamapis_offer_smoke.py` is a manual, disabled-by-default proof that the existing official WebSocket client, Step 2A parser, Step 2I foreground runner, and Step 2C in-memory pool work together against a real read-only SteamApis offer stream. It reads only the inherited process environment; it never loads `.env`.
