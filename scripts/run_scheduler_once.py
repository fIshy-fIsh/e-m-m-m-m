...
async def _run() -> None:
    ...
    heartbeat_result = await heartbeat_job_once(
        discord_config=DiscordWebhookConfig(webhook_url=None, dry_run=True),
        message="run_scheduler_once heartbeat",
    )
    cleanup_result = await cleanup_old_state_job_once()

    for result in [pipeline_result, heartbeat_result, cleanup_result]:
...
def main() -> None:
    """Run all scheduler jobs once using only mock and dry-run dependencies."""
