...
9. **TTL / config ownership**: NO scanner fresh_ttl numeric default is frozen in Phase 14A-R1. The 5-minute value in `scripts/steamdt_refresh_integration.py:59` is historical manual-script precedent only. Phase 14C will explicitly choose and document the scanner TTL under implementation authorization. InMemory and Redis must use identical policy semantics once chosen. No TTL/config code lands in R1.
