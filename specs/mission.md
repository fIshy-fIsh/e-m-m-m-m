...
Phase 16A freezes a recipe-first pre-screen discovery architecture that preserves the V1 read-only / strict BUFF valuation / no-fallback contracts and reuses the mature downstream calculation/safety stack. Implementation of the new production path (Phases 16B–16F) is separately gated; no representative campaign runs under the new path until Phase 16F passes. Phase 16A-R1 corrects three doc-only ambiguities: Souvenir is not a RecipeFamily structural identity axis, Top-N ranking does not multiply the live BUFF request budget (one active family per run; total BUFF page requests per run <= 10), and the K<=3 theoretical family-state space (~14M total) is analytic evidence and is not eagerly materialized.