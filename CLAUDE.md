...
8. 持久/普通本地开发 secret 继续使用现有 `.env` 规则；单独授权、case-bound 的 one-shot live 例外必须遵循 `docs/recipe-first-one-shot-live-operator-contract.md`，使用 repo 外、由 case SHA 派生的临时 handoff 文件，且不得写入 `.env`/argv/chat/Git/docs/history。
...
## 当前阶段指针
- 当前仓库权威：branch `feature/phase17d-r2-secret-handoff-prelive-freeze`，HEAD `a7ce9c019e3c1e6277ed29bbb58c53f4fbefe7e6`（`freeze phase17d r2 secret handoff live case`）。
- 当前状态：`PHASE17D_R2_SECRET_HANDOFF_PRELIVE_CASE_FROZEN_READY_FOR_NEW_AUTHORIZATION`，但 **R2 LIVE = FROZEN / AUTHORIZATION ON HOLD**；R2 未授权，child launch=0，secret access=0。外部 case（本机记录、非 Git 可复算事实）：`C:\Users\lijie\AppData\Local\Temp\cs2-phase17d-r2\phase17d_r2_case.json`，recorded SHA-256 `c929d234e8a7f693261ac7d2d48cb9b77004cb844f8a9cfeb6a4b5ec28e386e4`。
- HOLD 原因：Phoenix prescreen 为 19 unique names，项目 chunk=10 导致 10+9 两次立即 batch；官方 `PRICE_BATCH=1/min`，项目 effective window=65s（+5s buffer），limiter fail-fast 且无 inter-chunk pacing。`PRESCREEN_BATCH_CHUNK_SIZE=10` 是 project/smoke policy，不是官方 max；官方每批最大 names 仍 UNKNOWN。任何 R2 live 授权前必须离线完成 rate-compatible prescreen 设计并重新 freeze case。
- 原 Phase17D：actual CLI 仅执行一次，terminal `incomplete_or_provider / EXTERNAL_PROVIDER_FAILURE`，acceptance `C — INCONCLUSIVE_PROVIDER`；授权已 consumed / 永久关闭，不得复用。R1 offline diagnosis=`EXPECTED_FAIL_CLOSED_PROVIDER_FAILURE`，未发现 deterministic runtime defect。必须同时读取 `docs/phase17d-bounded-live-validation-evidence.md` 与 `docs/phase17d-bounded-live-validation-evidence-errata.md`：observed exit=`NOT_OBSERVED`，code-derived expected exit=2。
- 已完成：Phase16A–G；Phase17A design freeze；Phase17B runtime；Phase17C offline E2E；Phase17C-R1 collection-scoped discovery repair；Phase17D class-C observation；Phase17D-R1 diagnosis；Phase17D-R2 launcher/operator-contract freeze。
- Production recipe-first default=`OFF`；goods-first unchanged；Phase17E cutover=`NOT_STARTED`；Phase15C/Phase17F campaign=`NOT_STARTED`/deferred。下一安全任务：offline SteamDT documentation/rate-limit reconciliation + rate-compatible prescreen design；ZERO provider request / ZERO secret access / ZERO live authorization。
- One-shot launcher：`scripts/run_recipe_first_authorized_once.py`；standing contract：`docs/recipe-first-one-shot-live-operator-contract.md`。Secret file != authorization；Popen 成功返回 handle 才 consumed；无自动 retry/second invocation/repair-rerun。
