...
## 当前阶段指针
- 当前已完成阶段：`PHASE_16G_R7_LIVE_VALIDATED`（branch `feature/recipe-first-steamdt-live-validation`；pre-live Commit F `ce546730e4a34bf53e3973e5a30508361f51355e`；CI run `34083045096` SUCCESS；result schema v2；完成 commit `940127ff4d8f7d6a55aba4128670c059acd7e731`；完成 CI run `34085292093` SUCCESS）。
- 当前已完成实现阶段：`PHASE_17B_OPT_IN_RECIPE_FIRST_ONE_SHOT_RUNTIME_COMPLETE`（branch `feature/recipe-first-runtime-integration`；separate `scripts/run_recipe_first_scan_once.py`；explicit `--enable-recipe-first` / default-disabled；preview zero-market-client；runtime contracts + bounded single-stratum coordinator + real mature downstream reuse；Phase 17C NOT STARTED）。
- Phase 17B boundary：ZERO SteamDT/BUFF HTTP during implementation；production recipe-first default remains OFF；goods-first `LiveScannerOrchestrator` + `scripts/run_live_scan_once.py` byte-stable；no Phase16G harness import；FRESH_ONLY cache reads only / no scanner writeback；no scheduler/Discord/PostgreSQL/auto-buy/auto-trade；Phase15C NOT STARTED。
- 当前已完成设计阶段：`PHASE_17A_RECIPE_FIRST_RUNTIME_INTEGRATION_DESIGN_FROZEN`（branch `feature/recipe-first-runtime-integration-design`；commit `9e7efa3ace4aba354f401bf2fbd9f039ecfa479b`；CI run `34095539986` SUCCESS）。
- Phase 16G-R7 live evidence：one operator-authorized bounded attempt；case SHA `266bbd0f4df64bd947c4d081a0b978d51a03a221691c2e2acc8987fe890f5e68`；result SHA `9a0aa4ac2572018af65af3692f4f9db1e374efa2c7ad41723eaea11289a8f17e`；prescreen 9/9；BUFF 1 page / 10 listings / 10 compatible / 0 incompatible；real family-constrained search states=1 / selections=1；outputs `AUG | Chameleon (Field-Tested)` + `AWP | Asiimov (Battle-Scarred)` probability 0.5 each，floats `0.20962120840946835` / `0.523778781791528`，wears Field-Tested / Battle-Scarred；final strict `steamdt:buff` quotes CNY 524.49 / 505.77；structural fields preserved；batch=1 / BUFF=1 / singles=2；no retry/pagination/polling/fallback。Phase 16G 仅证明接口/路径正确，不证明盈利或风险通过；不再授权新的 Phase 16G live run。
- Phase 17A 冻结：entrypoint 选 `scripts/run_recipe_first_scan_once.py`（future Phase 17B），explicit opt-in / default-disabled；goods-first 脚本保持 byte-stable；替换 only discovery brain，reuse targeted acquisition + family-constrained concrete search + FRESH_ONLY run-scoped final valuation + EV/ROI/risk + report；one active family per run；family #2 only before any BUFF dispatch starts；prescreen economics 仅作 ranking/pruning，永不替代 final exact SteamDT-BUFF valuation；Phase 17B/C/D/E/F 分阶段 gated；production default 在 Phase 17D 之前保持 OFF；Phase 15C 仍 deferred until Phase 17F side-by-side re-entry。
- 阶段：`PHASE_16F_R2_FAMILY_METADATA_CONTRACT_VALIDATED`（authoritative v3 RecipeFamily-bound anonymous BUFF validation；branch `feature/recipe-first-live-buff-interface-validation-r2` from R1 `16562c3`）。
- Phase 16F-R2 correction：pinned metadata proves goods_id `33960` / `AK-47 | Redline (Field-Tested)` is `The Phoenix Collection` / `Classified` / normal / non-Souvenir / float 0.1–0.7, not the historical v1/v2 hardcoded `The 2018 Nuke Collection` / `Restricted`; historical generic BUFF exact-identity/acquisition evidence remains valid, while the old RecipeFamily-bound fixture claim is INVALID / SUPERSEDED。
- Phase 16F-R2 authoritative case：`build_recipe_family(Classified, normal, The Phoenix Collection ×10)` -> family_hash `45bfd0f0d3e7405588acdcf742d980577eed4963382c2fde31632fc43db52516`；`compute_recipe_family_geometry` -> Covert, 2 unique structural finishes, exact Fraction sum 1；v3 metadata contract runs during prepare and execute before HTTP；every enriched input/provenance is rechecked after HTTP。
- Phase 16F-R2 live evidence：one separately authorized anonymous GET only；attempted=1、dispatched=1、budget_exceeded=False；10 listings、10 candidate accepted、10 metadata resolved、10 family-compatible enriched inputs；classification=`validated`；ZERO SteamDT/Discord/Redis/PostgreSQL/scheduler；no retry/pagination/fallback；production recipe-first remains OFF；Phase 15C campaign NOT STARTED。
- Phase 16F-R1 artifact decisions remain active：`repository_commit_oid` stores exact Git OID；persisted canonical bytes have no newline；one authoritative `case_sha256`；v1/v2 artifacts rejected by v3 loader。
